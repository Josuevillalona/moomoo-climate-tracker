...
export class FundingService {
  ...
  static async getDashboardMetrics(): Promise<ApiResponse<DashboardMetrics>> {
    ...
    try {
      ...
      console.log('FundingService.getDashboardMetrics: Fetching basic metrics...');
      ...
      console.log('FundingService.getDashboardMetrics: Fetching sector data...');
      ...
      console.log('FundingService.getDashboardMetrics: Fetching country data...');
      const { data: countryData, error: countryError } = await measureQuery(
      ...
      console.log('FundingService.getDashboardMetrics: Fetching investor data...');
      const { data: investorData, error: investorError } = await measureQuery(
      ...
      console.log('FundingService.getDashboardMetrics: Calculating optimized metrics...');
      ...
      console.log('FundingService.getDashboardMetrics: Calculated optimized metrics:', metrics);
  ...
  static async getRecentDeals(limit: number = 5): Promise<ApiResponse<FundingDeal[]>> {
    console.log('FundingService.getRecentDeals: Starting optimized version with limit:', limit);
    
    try {
      console.log('FundingService.getRecentDeals: Querying Supabase with optimized query...');
  ...
  static async getDealsWithFilters(filters: DealFilters): Promise<ApiResponse<PaginatedResponse<FundingDeal>>> {
    try {
      console.log('FundingService.getDealsWithFilters: Starting optimized query with filters:', filters);
      ...
      query = query.order('date_announced', { ascending: false, nullsFirst: false });

      // Apply pagination with range
      query = query.range(offset, offset + limit - 1);

      console.log('FundingService.getDealsWithFilters: Executing optimized query...');
      const { data: deals, error, count } = await measureQuery(
        'filtered-deals-paginated',
        () => query,
        { 
          queryType: 'filtered-pagination',
          filters: Object.keys(filters).filter(key => filters[key as keyof DealFilters] !== undefined),
          limit,
          offset,
          expectedRows: limit
        }
      ...
      if (error) {
        console.error('FundingService.getDealsWithFilters: Database error:', error);
        throw new ApiException(
          ApiErrorType.DATABASE_ERROR,
  ...
  private static calculateOptimizedMetrics(
    basicMetrics: Array<{id: number, amount_raised: number | null, company_name: string, date_announced: string | null, created_at: string}>,
    sectorData: Array<{climate_sub_sector: string | null, amount_raised: number | null}>,
    countryData: Array<{geography_country: string | null, amount_raised: number | null}>,
    investorData: Array<{lead_investors: string | null, other_investors: string | null}>
  ): DashboardMetrics {
    console.log('FundingService.calculateOptimizedMetrics: Processing data sets...');
    
    // Basic metrics from main dataset
    const totalDeals = basicMetrics.length;
    const totalFunding = basicMetrics.reduce((sum, deal) => sum + (deal.amount_raised || 0), 0);
    const totalCompanies = new Set(basicMetrics.map(deal => deal.company_name)).size;
    ...
    console.log('FundingService.calculateOptimizedMetrics: Calculated metrics:', {
      totalDeals,
      totalFunding,
      totalCompanies,
      totalInvestors,
